 ...
 **1. Jelaskan manfaat dari penggunaan JavaScript dalam pengembangan aplikasi web!**
 - JavaScript itu bahasa pemrograman yang  penting dalam pengembangan web karena memungkinkan pengembangan frontend dan backend yang interaktif. Manfaat utamanya termasuk:
    - Interaktiv: JavaScript memungkinkan pembuatan halaman web yang interaktif. Pengguna bisa berinteraksi dengan halaman secara dinamis tanpa perlu memuat ulang halaman.
    - Kemudahan penggunaan: Banyak framework dan library yang tersedia, yang membuat pengembangan web menjadi lebih mudah dan efisien.
    - Real-time updates: Dengan menggunakan AJAX dan WebSockets, JavaScript memungkinkan pengembangan aplikasi yang dapat memperbarui data secara real-time tanpa perlu memuat ulang halaman.
    - Kaya fitur: JavaScript mendukung fitur seperti drag-and-drop, zoom, dan slide yang dapat meningkatkan pengalaman pengguna.

 **2. Jelaskan fungsi dari penggunaan await ketika kita menggunakan fetch()! Apa yang akan terjadi jika kita tidak menggunakan await?**
 - Ketika menggunakan fetch(), await digunakan untuk menunggu promis dari fetch() terselesaikan, yang berarti menunggu data yang diminta selesai diambil sebelum melanjutkan eksekusi kode berikutnya. Jika tidak menggunakan await, eksekusi kode akan berlanjut tanpa menunggu hasil fetch(), yang bisa menyebabkan JavaScript mengakses data yang belum siap atau belum diterima, sehingga menghasilkan error atau data yang tidak konsisten.

 **3. Mengapa kita perlu menggunakan decorator csrf_exempt pada view yang akan digunakan untuk AJAX POST?**
 - Decorator csrf_exempt digunakan untuk menonaktifkan proteksi CSRF (Cross-Site Request Forgery) untuk view tertentu. Ini diperlukan dalam kasus AJAX POST ketika mengirimkan data dari klien ke server dari luar form yang di-render oleh Django, atau ketika menggunakan API dari domain lain, di mana token CSRF tidak dapat diatur dengan mudah pada header request.

 **4. Pada tutorial PBP minggu ini, pembersihan data input pengguna dilakukan di belakang (backend) juga. Mengapa hal tersebut tidak dilakukan di frontend saja?**
 - Pembersihan data di backend juga diperlukan untuk memastikan keamanan aplikasi karena tidak semua pengguna akan menggunakan frontend yang disediakan, misalnya, jika mereka mengakses API langsung. Pengguna juga dapat memodifikasi JavaScript atau menghentikan JavaScript di browser mereka, sehingga melewati validasi atau sanitasi yang dilakukan di frontend.Lalu Melakukan pembersihan baik di frontend maupun backend memastikan bahwa data yang masuk ke sistem kita adalah aman dan telah divalidasi, mengurangi risiko bug atau celah keamanan.

 **5. Jelaskan bagaimana cara kamu mengimplementasikan checklist di atas secara step-by-step (bukan hanya sekadar mengikuti tutorial)!**
 - Pertama, Saya menambahkan import csrf_exempt dari django.views.decorators.csrf dan require_POST dari django.views.decorators.http untuk memastikan bahwa fungsi view saya dapat menerima POST request dan dikecualikan dari proteksi CSRF. Saya membuat fungsi baru add_product_rating_ajax yang menerima data name, price, rating, feedback dari request.POST, dan membersihkan data tersebut menggunakan strip_tags untuk mencegah serangan XSS. Fungsi ini kemudian menyimpan data ke dalam model Product.
 - Lalu, saya menambahkan URL baru dalam urls.py yang mengarah ke fungsi add_product_rating_ajax untuk menangani POST request yang dikirim oleh AJAX.  Saya juga menghapus product_rating = Product.objects.filter(user=request.user) dan 'product_rating': product_rating. Kemudian saya mengubah baris pertama view untuk show_json dan show_xml seperti data = Product.objects.filter(user=request.user)
 - Selanjutnya, saya mengubah main.html dengan menambahkan fungsi JavaScript yang memungkinkan pengambilan data dan pengiriman form. Saya menambahkan sebuah tombol yang ketika diklik akan memicu fungsi JavaScript showModal() untuk menampilkan modal. Di dalam modal ini ada sebuah form yang mengumpulkan informasi produk seperti nama produk, price, rating, feedback. Fungsi getProductEntries() menggunakan fetch() untuk mengambil data rating produk yang ditampilkan dalam format JSON dari server. Data ini kemudian ditampilkan pada halaman secara dinamis tanpa reload. Ketika form diisi dan tombol "Save" diklik, fungsi addProductRating() akan dipicu yang mengirimkan data form melalui AJAX POST ke server.
 - Setelah data produk berhasil disimpan ke database, halaman produk diperbarui tanpa memuat ulang dengan memanggil getProductEntries() lagi setelah POST berhasil. Saya menggunakan strip_tags() di views.py untuk membersihkan input dari pengguna.